...
<html>
    ...
    <body class="bg-light">
        ...
        if ($SendPesqUser) {
            ...
            if ($linha == 0) {
                $cadastro = $con->query("INSERT INTO horas (professor_id, ano_letivo_id, mes, semana1, semana2, semana3, semana4, semana5, total) VALUES ($professores_id, $ano_letivo, $mes, 0, 0, 0, 0, 0,0)");
                unset($consulta);
                unset($cadastro);
            }
        } else {
            $professores_id = $_SESSION["professor_id"];
            $nome = $con->query("SELECT nome FROM professores WHERE id=$professores_id");
            $nome_professor = $nome->fetch_row();
        ...
        <div class="container">
            ...
            <form  method="POST" action="equipe_salvar_horas1.php">
                <table class="table table-bordered table-hover p-0 m-0">
                    ...
                    <a href="equipe_cadastrar_horas.php" style="color:white; font-weight: bold;" class="btn btn-danger">VOLTAR</a>
                    &nbsp; &nbsp; 
