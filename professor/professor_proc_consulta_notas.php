...
<html>
    ...
    <body class="bg-light">
        ...
        if ($SendPesqUser) {
            ...
            $resultado = $con->query("SELECT * FROM alunos join notas on alunos.id = notas.aluno_id join turmas on notas.turmas_id = turmas.id where notas.turmas_id = $turma AND notas.professores_id = $professores_id AND notas.disciplina_id = $disciplina_id AND notas.trimestre_id = $trimestre AND notas.ano_letivo_id = $ano_letivo");
            $datas = $resultado->fetch_array();
        }
        ?>
        <div class="container-fluid">
            ...
            <form  id="formulario" method="POST" action="professor_salvar_nota.php">
                <table class="table table-bordered table-hover p-0 m-0">
                    ...
                    while ($rows = $result->fetch_array()) {
                        ...
                        <tr class="p-0 m-0">
                            ...
                            <td class="p-1 m-0 align-middle" style='text-align: center;'><input type="text" class="w-75 p-0 m-0 border-0 bg-light nota" style="text-align:center;" name="nota1[]" tabindex="1" value="<?php echo $rows['nota1']; ?>"></td>
...
// Fim da tabela
                    ...
                    ?>
                    <br>
                    <a href="professor_consultar_nota.php" style="color:white; font-weight: bold;" class="btn btn-danger">VOLTAR</a>
                    &nbsp; &nbsp; 
                    
                    &nbsp; &nbsp; 
                    
                    <button  type="submit" style="color:white; font-weight: bold;" name="alterar" id="alterar" class="btn btn-success">ENVIAR DADOS</button>
            </form>
            <br>
        </div>
